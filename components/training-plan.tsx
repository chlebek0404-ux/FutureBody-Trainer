                  ...
                  <article key={item.id} className={`${panelClass} overflow-hidden`}>
                    <div className="grid lg:grid-cols-[150px_1fr]">
                      ...
                      <div className="p-5">
                        ...
                        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4"><PlanField label="Serie" value={item.sets} type="number" onChange={(value) => updateItem(item.id, "sets", Number(value))}/><PlanField label="Powtórzenia" value={item.reps} onChange={(value) => updateItem(item.id, "reps", value)}/><PlanField label="Ciężar" value={item.load} onChange={(value) => updateItem(item.id, "load", value)}/><PlanField label="Przerwa (s)" value={item.restSeconds} type="number" onChange={(value) => updateItem(item.id, "restSeconds", Number(value))}/></div>
...
function PlanField({ label, value, type = "text", onChange }: { label: string; value: string | number; type?: string; onChange: (value: string) => void }) {
  return <label><span className="mb-1.5 block text-[8px] font-black uppercase tracking-wider text-black/30">{label}</span><input value={value} type={type} min={type === "number" ? 0 : undefined} onChange={(event) => onChange(event.target.value)} className="h-10 w-full rounded-xl bg-[#f4f4f2] px-3 text-[10px] font-bold outline-none focus:ring-1 focus:ring-black"/></label>;
}
